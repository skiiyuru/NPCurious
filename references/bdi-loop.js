...
socket.onSensing(async (data) => {
  // add/refresh parcels beliefs
  for (const p of data.parcels) {
    parcels.set(p.id, p)
  }
  // TODO: introduce uncertainity in beliefs
  // what we do here is too harsh
  // add probabilities, decay over time
  for (const p of parcels.values()) {
    if (data.parcels.map((p) => p.id).find((id) => id == p.id) == undefined) {
      parcels.delete(p.id)
    }
  }
})
...
function optionsGeneration() {
  ...
  for (const option of options) {
    if (option[0] == "go_pick_up") {
      ...
      let current_d = computeDistance({ x, y }, me)
      if (current_d < nearest) {
        best_option = option
        nearest = current_d
      }
      // consider current carrying? reward minus distance? other agents?
    }
  }
...
//     }
//     optionsGeneration();
//     await new Promise( res => setTimeout( res ) ); // release control to event loop so to process eventual updates of beliefs (me and parcels) and trigger new options generation
//     safer to use setTimeout
// }
  ...
  async push(predicate) {
    ...
    if (
      this.intention_queue.find(
        (i) => i.predicate.join(" ") == predicate.join(" ") // string matching
      )
    )
...
class BlindMove extends PlanBase {
  ...
  async execute(go_to, x, y) {
    while (me.x != x || me.y != y) {
      ...
      if (me.x && x > me.x) moved_horizontally = await socket.emitMove("right")
      ...
      if (me.y && y > me.y) moved_vertically = await socket.emitMove("up")
